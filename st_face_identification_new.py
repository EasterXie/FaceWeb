...
def create_connection():
    try:
        ...
        print('连接数据库失败/待创建数据库已经存在')
        return
    try:          
        conn.execute('''CREATE TABLE STUDENTS_DATA
...
#         CatchUsbVideo("识别人脸区域", 0, path_create, 10, classer="./haarcascade_profileface.xml", sign="profileface")
# #在服务器上备份一份
#         upload.upload_folder_to_server(path_create, remote_path, '123.249.37.195', 'root', 'Hadoop@123')
    ...
    if st.button('删除指定的人脸数据'):
        delete_folder(path_delete)


def camera_shot():
    train_name = st.text_input("输入生成训练集模型的文件名")
    print(f"file name is {train_name}")
